...
class GoogRequireParserPlugin {
  ...
  addGoogDependency(parser, request, addAsBaseJs) {
    ...
    const baseInsertPos = -1; //!isProductionLikeMode(this.options) ? -1 : null;
  ...
  addEs6LoaderDependency(parser) {
    ...
    // ES6 prefixing must happen after all requires have loaded otherwise
    // Closure library can think an ES6 module is calling goog.provide/module.
    const baseInsertPos = 0; //!isProductionLikeMode(this.options) ? 0 : null;
    const sourceLength = true //!isProductionLikeMode(this.options)
