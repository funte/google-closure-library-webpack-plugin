...
const BadRequire = require('../src/errors/BadRequire');
const DeprecateWarning = require('../src/errors/DeprecateWarning');
...
const MissingParameterError = require('../src/errors/MissingParameterError');
const MultiCallingError = require('../src/errors/MultiCallingError');
const ModifyImplicitNamespaceWarning = require('../src/errors/ModifyImplicitNamespaceWarning');
const ModifyRequiredNamespaceWarning = require('../src/errors/ModifyRequiredNamespaceWarning');
const NamespaceConflictError = require('../src/errors/NamespaceConflictError');
const NamespaceOutModuleError = require('../src/errors/NamespaceOutModuleError');
const UnexpectCallingError = require('../src/errors/UnexpectCallingError');
...
describe('Test ClosureModuleParserPlugin', () => {
  ...
  describe('detect base.js', () => {
    it('other file should not be', () => {
      tree.clear();
      const module = tree.loadModule(tree.depsfile,
        `/*\n` +
        ` * @provideGoog\n` +
        ` */\n` +
        `var goog = goog || {};\n`
    ...
    it('detect by path', () => {
      tree.clear();
      const module = tree.loadModule(tree.basefile);
      expect(tree.errors).to.empty;
      ...
      const info = module.provides.get('goog');
      expect(info).to.exist;
      expect(info.namespace).to.equal('goog');
  ...
  describe('detect CommonJS', () => {
    it('detect file extension', () => {
      tree.clear();
      const module = tree.loadModule('path/to/module.cjs', ``);
      expect(tree.errors).to.empty;
    ...
    it('detect typeof for module', () => {
      tree.clear();
      const module = tree.loadModule('path/to/module', `typeof module;\n`);
      expect(tree.errors).to.empty;
      expect(module.type).to.equal(ModuleType.COMMONJS);
    });
  ...
  describe('detect ES module', () => {
    it('detect file extension', () => {
      tree.clear();
      const module = tree.loadModule('path/to/module.mjs', ``);
      expect(tree.errors).to.empty;
  ...
  describe('parse goog.define', () => {
    ...
    it('test with defs option', () => {
      ...
      tree.env.defs = new Map();
      tree.env.defs.set('name', 'false');
      tree.env.defs.set('COMPILED', 'false');
      tree.env.defs.set('goog.DEBUG', 'true');
  ...
  describe('parse module', () => {
    describe('parse goog.declareModuleId', () => {
      it('parse goog.declareModuleId', () => {
        ...
        expect(info).to.exist;
        expect(info.namespace).to.equal('a');
